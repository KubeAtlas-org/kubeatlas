...
- **macOS** — `make run` / `build` / `lint` and the seed clusters work; install `kwokctl`/`kubectl` (and, for `make brand-png`, `librsvg` + `imagemagick` + `fonttools` + `pillow`) via Homebrew, with Docker running. `make stop` is Linux-only (it reads `/proc`) — use Ctrl-C or `kill` instead.
...
```bash
make seed-small    # spin up the small KWOK cluster (kubeatlas-test-small) + switch kubectl to it
make run           # hot-reload dev server (via air) against your current kubeconfig context
make teardown-small # delete the small KWOK cluster
make stop          # stop ONLY this repo's air/tmp/main (Linux-only)
```

`make seed-small` provisions an in-process KWOK cluster using kwokctl's **binary** runtime — no Docker-in-Docker — so a clone goes straight to a live server (see [Test clusters](#test-clusters) for the prod/incident variants). `make run` sets `STATIC_DIR=public`, so the frontend is served from disk: edit anything under `public/` and reload the browser, no rebuild.
...
```bash
make seed-small             # kubeatlas-test-small — minimal ~2 nodes / ~7 pods (2 Deployments +
                            #   1 DaemonSet, 1 namespace): a lightweight cluster to drive the UI
...
A separate `.github/workflows/dev-tools.yml` exercises the OS- and tool-version-dependent developer/distribution tooling: the brand-png pipeline across Linux (ImageMagick v6) and macOS (v7), checking output dimensions and byte-for-byte reproducibility; the KWOK seed/teardown scripts; and the install scripts (`docs/install.sh` on Linux, `docs/install.ps1` on Windows), which install the latest release end-to-end on a real runner. It runs only when that tooling changes (`scripts/`, `.dev/`, `makefile`, `.devcontainer/`, `docs/install.*`, brand and font assets) and is never a required check.
